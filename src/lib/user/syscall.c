...
int report_mem_usage(void) {
    int usage;
    asm volatile (
        "int $0x30"  // Trigger interrupt 0x30 to call the syscall handler
        : "=a" (usage)  // Output the result into the eax register
        : "a" (SYS_REPORT_MEM_USAGE)  // Pass the syscall number in eax register
    );
    return usage;  // Return the result (memory usage)
}